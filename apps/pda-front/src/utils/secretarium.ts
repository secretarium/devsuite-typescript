...
export const scpClient = new SCP({
...
const initConnection = async (options: Pick<SecretariumQueryOptions, 'key'> = {}) => {
    ...
    if (connectionKey && key && await connectionKey.getRawPublicKeyHex() !== await key.getRawPublicKeyHex()) {
        ...
        await new Promise((resolve, reject) => {
            ...
            const timer = setInterval(() => {
                if (!scpClient.isConnected() && scpClient.state === Constants.ConnectionState.closed) {
                    clearTimeout(bail);
                    clearInterval(timer);
                    resolve(true);
                }
    ...
    if (!connectionKey)
        throw new Error('Missing Secretarium key');

    const [node, trustKey] = connectionInfo ?? [];
    if (!node)
        throw new Error('Missing Secretarium node or trust key');

    if (scpClient.state === Constants.ConnectionState.closed) {
        console.debug('Connecting to Secretarium...');
        await scpClient.connect(node, connectionKey, trustKey);
    }
    ...
    await new Promise((resolve, reject) => {
        ...
        const timer = setInterval(() => {
            if (scpClient.state === Constants.ConnectionState.secure) {
                clearTimeout(bail);
                clearInterval(timer);
                resolve(true);
            }
...
export function useSecretariumClient() {
    if (scpClient.state === Constants.ConnectionState.closed)
        return [scpClient, initConnection()] as const;
    else
        return [scpClient, Promise.resolve(scpClient)] as const;
}

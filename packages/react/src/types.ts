...
import type { Connector } from '@secretarium/connector';
...
export type QueryDataFunction<TData> = () => Promise<TData | void>;
...
export type Cache = {
    get: <TData>(name: string) => Promise<TData | undefined>
    set: <TData>(name: string, data: TData) => Promise<void>
...
export type RetryOn = (({ attempt, error, response }: RetryOpts) => Promise<boolean>) | number[]
export type RetryDelay = (({ attempt, error, response }: RetryOpts) => number) | number

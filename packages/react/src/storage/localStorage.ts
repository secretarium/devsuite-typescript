...
const cacheName = 'useSecretariumLedger';
...
const getLocalStorage = ({ cacheLife }: { cacheLife: number }): Cache => {
    ...
    const get = async <TData>(responseID: string) => {
        ...
        // });
        return cache[responseID].response as TData;
    };

    const set = async <TData>(responseID: string, response: TData): Promise<void> => {
        const cache = getCache();
        cache[responseID] = {
